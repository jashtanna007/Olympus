-- Add database-backed undo/redo for auction bids and player completion actions.
-- Apply after 20260727_fix_auction_runtime.sql.
...
CREATE INDEX IF NOT EXISTS auction_action_history_stack_idx
  ON public.auction_action_history (is_undone, id DESC);

ALTER TABLE public.auction_action_history ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Auction admins can view action history"
  ON public.auction_action_history;

CREATE POLICY "Auction admins can view action history"
  ON public.auction_action_history
  FOR SELECT
  TO authenticated
  USING (public.is_auction_admin());

REVOKE INSERT, UPDATE, DELETE ON public.auction_action_history FROM authenticated;
GRANT SELECT ON public.auction_action_history TO authenticated;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE pubname = 'supabase_realtime'
      ...
      AND tablename = 'auction_action_history'
  ) THEN
    ALTER PUBLICATION supabase_realtime ADD TABLE public.auction_action_history;
  END IF;
END;
$$;

CREATE OR REPLACE FUNCTION public.auction_discard_redo_history()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.auction_history_state()
...
-- Recreate the bid command so every accepted bid becomes one undoable action.
...
-- Recreate completion so sold/unsold actions can be restored atomically.
    ...
    IF v_outcome = 'sold' THEN
      ...
      ELSE
        ...
        VALUES (
          ...
          v_member_before->>'institute_email',
...
REVOKE ALL ON FUNCTION public.auction_discard_redo_history() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.auction_history_state() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.auction_undo_last() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.auction_redo_last() FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.auction_history_state() TO authenticated;
GRANT EXECUTE ON FUNCTION public.auction_undo_last() TO authenticated;
GRANT EXECUTE ON FUNCTION public.auction_redo_last() TO authenticated;
